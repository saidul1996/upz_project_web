...
{{__('Chairman Admin')}}
@endsection
@section('chairmanadminmenu')
active show-sub
@endsection
@section('chairmanadminlist')
active
@endsection
@section('css')
...
<nav class="breadcrumb sl-breadcrumb">
    ...
    <span class="breadcrumb-item active">{{__('Chairman')}}</span>
</nav>
    ...
    <div class="card pd-20 pd-sm-40">
      <div class="row">
        ...
        {{--<div class="col-5 text-right">
            <a class="btn btn-info" href="{{ route('admin.chairmanAdmin.create') }}">{{ __('Create New') }}</a>
        </div>--}}
      </div>
        ...
        <table id="example" class="table display responsive nowrap">
            <thead>
              <tr>
                ...
                <th class="wd-15p text-center">{{ __('Name') }}</th>
                <th class="wd-15p text-center">{{ __('Phone') }}</th>
                <th class="wd-10p text-center">{{ __('District') }}</th>
                <th class="wd-10p text-center">{{ __('Upazilla') }}</th>
                <th class="wd-15p text-center">{{ __('Union') }}</th>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="text-center">{{ $data->union->upazilla->district->name??'' }}</td>
                <td class="text-center">{{ $data->union->upazilla->name??'' }}</td>
                <td class="text-center">{{ $data->union->name??'' }}</td>
                ...
                <td class="text-center">
                    <a title="Show Item" class="mr-2" href="{{route('admin.chairmanAdmin.show',$data->id)}}"><i class="text-info menu-item-icon icon icon ion-ios-eye tx-24"></i></a>
                    @if(App\Models\Admin::where([['email', $data->email],['name', $data->name]])->count())
                    <a title="Update Item" class="mr-2" href="{{route('admin.chairmanAdmin.edit',$data->id)}}"><i class="text-success menu-item-icon icon ion-compose tx-24"></i></a>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <form action="{{route('admin.chairmanAdmin.destroy',$data->id)}}" method="POST">
                                    @csrf
                                    @method('DELETE')
