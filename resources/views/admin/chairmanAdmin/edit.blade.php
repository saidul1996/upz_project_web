...
<nav class="breadcrumb sl-breadcrumb">
    ...
    <span class="breadcrumb-item active">{{__('Chairman Admin')}}</span>
</nav>
    ...
    <div class="card pd-20 pd-sm-40">
        <div class="row">
            ...
            <div class="col-5 text-right">
                <a class="btn btn-info" href="{{ route('admin.chairmanAdmin.index') }}">{{__('View All')}}</a>
            </div>
        </div>
        @include('partials.message')
        <form action="{{route('admin.chairmanAdmin.update',$data->id)}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-layout">
                <div class="row mg-b-25">
                    <div class="col-lg-4">
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" value="{{ $data->union->upazilla->district->name??'' }}" readonly>
                        </div>
                    </div><!-- col-4 -->
                    <div class="col-lg-4">
                        <div class="form-group">
                            <label class="form-control-label">{{ __('Upazilla') }}: </label>
                            <input class="form-control" type="text" value="{{ $data->union->upazilla->name??'' }}" readonly>
                        </div>
                    </div><!-- col-4 -->
                    <div class="col-lg-4">
                        <div class="form-group">
                            <label class="form-control-label">{{ __('Union') }}: </label>
                            <input class="form-control" type="text" value="{{ $data->union->upazilla->name??'' }}" readonly>
                        </div>
                    </div><!-- col-4 -->
                    <div class="col-lg-4">
                ...
                <div class="form-layout-footer">
                    ...
                    <a href="{{ route('admin.chairmanAdmin.index') }}" class="btn btn-secondary">{{ __('Cancel') }}</a>
